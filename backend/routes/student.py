...
from services.exam_service import (
    service_get_exam, service_get_active_exams,
    service_get_scheduled_exams, service_search_exams,
)
...
def active_exams(db: Session = Depends(get_db),
    ...
    """Exams currently open for attempt."""
    return service_get_active_exams(db)
...
def scheduled_exams(db: Session = Depends(get_db),
    ...
    return service_get_scheduled_exams(db)
...
def my_score(exam_id: int, db: Session = Depends(get_db),
    ...
    if not sub or not sub.is_completed:
        from fastapi import HTTPException
        raise HTTPException(status_code=404,
                            detail="No completed submission found")
    return sub
