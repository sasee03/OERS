      ...
      <div className="max-w-7xl mx-auto px-8 h-14 flex items-center justify-between">
        ...
        {!minimal && user?.role === "admin" && (
          <div className="flex gap-6">
            {[["Dashboard","/admin"],["Exams","/admin/exams"],["Results","/admin/results"],["Students","/admin/students"]].map(([label,path]) => (
              <Link key={path} to={path} className="text-[10px] uppercase tracking-[0.2em] text-zinc-400 hover:text-black transition-colors">{label}</Link>
            ))}
          </div>
        )}
        {!minimal && user?.role === "student" && (
          <div className="flex gap-6">
            <Link to="/student" className="text-[10px] uppercase tracking-[0.2em] text-zinc-400 hover:text-black transition-colors">My Exams</Link>
          </div>
        )}
        {user && (
