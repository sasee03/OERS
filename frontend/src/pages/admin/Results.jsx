        ...
        {loading ? <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-400">Loading...</p> : (
          <div className="border border-zinc-100">
            <table className="w-full">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={6} className="text-center py-16 text-[10px] uppercase tracking-[0.2em] text-zinc-400">No results found</td></tr>
                ) : filtered.map((r, i) => (
                  <tr key={r.submission_id} className="border-b border-zinc-50 hover:bg-zinc-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-[10px] uppercase tracking-[0.2em] ${r.percentage >= 60 ? "text-black" : "text-zinc-400"}`}>{r.percentage}%</span>
                    </td>
                    <td className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] text-zinc-400">{new Date(r.submitted_at).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
