...
export default function ModifyExam() {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      setEmails(""); getCandidates(id).then(r => setCandidates(r.data))
  ...
  const handleToggleActive = async () => {
    await updateExam(id, { is_active: !exam.is_active }); getExam(id).then(r => setExam(r.data))
  }
  ...
  const tabs = ["details","assign","questions","candidates"]
        ...
        <div className="flex gap-0 border-b border-zinc-100 mb-8">
          {tabs.map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`text-[10px] uppercase tracking-[0.2em] px-6 py-3 transition-colors border-b-2 -mb-px ${tab === t ? "border-black text-black" : "border-transparent text-zinc-400 hover:text-black"}`}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div className="max-w-2xl">
          {tab === "details" && (
            <div className="flex flex-col gap-8">
              ...
              <button onClick={handleExtendDate} className="bg-black text-white text-[10px] uppercase tracking-widest px-6 py-3 hover:bg-zinc-800 transition-all w-fit">
                Update
              </button>
            </div>
          )}
          ...
          {tab === "assign" && (
            <div className="flex flex-col gap-8">
              <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-400">Enter emails separated by comma or new line</p>
              <div className="flex flex-col gap-2">
              ...
              <button onClick={handleAssign} className="bg-black text-white text-[10px] uppercase tracking-widest px-6 py-3 hover:bg-zinc-800 transition-all w-fit">
                Assign Students
              </button>
            </div>
          )}
              ...
              {questions.map((q, i) => (
                <div key={q.id} className="border border-zinc-100 p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4 ml-10">
                    {["a","b","c","d"].map(opt => (
                      <span key={opt} className={`text-[10px] uppercase tracking-[0.2em] px-3 py-1 ${q.correct_answer === opt.toUpperCase() ? "bg-black text-white" : "border border-zinc-100 text-zinc-500"}`}>
          ...
          {tab === "candidates" && (
            <div className="border border-zinc-100">
              <table className="w-full">
                ...
                <tbody>
                  {candidates.length === 0 ? (
                    <tr><td colSpan={4} className="text-center py-16 text-[10px] uppercase tracking-[0.2em] text-zinc-400">No students assigned yet</td></tr>
                  ) : candidates.map((c, i) => (
