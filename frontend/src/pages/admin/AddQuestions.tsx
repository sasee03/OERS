...
export default function AddQuestions() {
  ...
  const handleChange = (field: keyof QuestionDraft, value: string) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(Array.isArray(d) ? (d[0] as { msg: string }).msg : (d as string) || "Failed to save questions")
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              {(["a","b","c","d"] as const).map(opt => (
                <div key={opt} className="flex flex-col gap-2">
                  ...
                  <input value={q[`option_${opt}` as keyof QuestionDraft] as string}
                    onChange={e => handleChange(`option_${opt}` as keyof QuestionDraft, e.target.value)}
