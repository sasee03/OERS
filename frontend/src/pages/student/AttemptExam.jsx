...
export default function AttemptExam() {
  ...
  const handleSubmit = useCallback(async (auto = false) => {
    ...
    try {
      const payload = {}; questions.forEach(q => { if (answers[q.id]) payload[q.id] = answers[q.id] })
      await submitExam(id, { answers: payload }); navigate(`/student/exams/${id}/score`)
    } catch (err) { setError(err.response?.data?.detail || "Submission failed"); setSubmitting(false) }
  }, [submitting, answers, questions, id, navigate])

  useEffect(() => {
    Promise.all([getExam(id), getQuestions(id)]).then(([e, q]) => {
      setExam(e.data); setQuestions(q.data)
      setTimeLeft(Math.max(0, Math.floor((new Date(e.data.end_time) - new Date()) / 1000)))
  ...
  if (!exam || questions.length === 0)
    return <div className="min-h-screen bg-white"><Navbar minimal /><p className="text-center pt-24 text-[10px] uppercase tracking-[0.2em] text-zinc-400">Loading exam...</p></div>
        ...
        <div>
          ...
          <div className="border border-zinc-100 p-8 mb-6">
            ...
            <div className="flex flex-col gap-3">
              {["A","B","C","D"].map(opt => (
                <label key={opt} className={`flex items-center gap-4 px-5 py-4 border cursor-pointer transition-all ${answers[q.id] === opt ? "border-black" : "border-zinc-100 hover:border-zinc-300"}`}>
                  <input type="radio" name={`q-${q.id}`} value={opt} checked={answers[q.id] === opt}
                    onChange={() => setAnswers({ ...answers, [q.id]: opt })} className="hidden" />
                  <span className={`w-7 h-7 flex items-center justify-center text-xs font-bold transition-all ${answers[q.id] === opt ? "bg-black text-white" : "border border-zinc-200 text-zinc-400"}`}>{opt}</span>
